...
describe('Miscellaneous Publication API Test - POST Requests [Tag: API Testing]', () => {
    ...
    it('TC-51: Verify Response When isPublished Field is Missing in the Miscellaneous Publication Request [Tag: API Testing] [Tag: Negative Testing]', async () => {
        // Given: A miscellaneous publication missing the isPublished field
        ...
        // When: Sending a POST request missing the isPublished field
        const response = await request.post('/miscPublication')
                                      .send(miscPublication)
                                      .set('Accept', 'application/json');

        // Then: Expect a failure response with status 400 due to the missing isPublished field
