...
describe('Event API Test - POST Requests [Tag: API Testing]', () => {
    ...
    it('TC-103: Verify Response When UbicationID Field is Missing in the Event Request [Tag: API Testing] [Tag: Negative Testing]', async () => {
        // Given: UbicationID field is missing in the event request
        const event = {
            ...
            userId: getCreatedUserId(),  // User ID is provided for clarity
            cost: 50
        };

        // When: Posting to create an event without the UbicationID field
        const response = await request.post('/event')
                                      .send(event)
                                      .set('Accept', 'application/json');

        // Then: Expect a failure due to missing UbicationID field, with status 400 and appropriate error message
