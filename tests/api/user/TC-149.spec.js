...
describe('User API Test - POST Requests [Tag: API Testing]', () => {
    ...
    it('TC-149: Verify the user is correctly created with the student role [Tag: API Testing] [Tag: Acceptance Testing]', async () => {
        // Given: User data for a student role
        const userData = {
            "name": "example",
        ...
        // When: Sending a POST request to create a user with student role
        const response = await request.post('/user')
                                      .send(userData)
                                      .set('Content-Type', 'application/json')
                                      .expect('Content-Type', /json/);

        // Then: Expect the user creation to be successful and role to be student
