...
describe('Publish Event Form Tests [Tag: GUI Testing][Tag: Functional Testing]', function () {
  ...
  it('TC-152: should show validation errors when the price is negative [Tag: GUI Testing] [Tag: Negative Testing] [Tag: Functional Testing]', async function () {
    // Given the user is on the Publish Event form page and enters a negative price
    ...
    // Then the user should see validation errors about the invalid price
    const isMessageDisplayed = await alertSnackBarPage.verifyMessage('Parámetros no válidos en la aplicación');
    assert.strictEqual(isMessageDisplayed, true, 'La validación de precio negativo no fue mostrada correctamente');
  });
});